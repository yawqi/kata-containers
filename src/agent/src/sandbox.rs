...
impl Sandbox {
    ...
    // #[instrument]
    // pub async fn setup_shared_namespaces(&mut self, shared_userns: bool, shared_netns: bool, uid_mappings: Vec<LinuxIDMapping>, gid_mappings: Vec<LinuxIDMapping>) -> Result<bool> {
    //     // TODO try to find out whether need to setup shared userns and netns
    //     // noted the userns needs to be created first.
    //     if shared_userns {
    //         self.shared_userns = Some(Namespace::new(&self.logger)
    //             .get_user(uid_mappings, gid_mappings)
    //             .setup(None)
    //             .await
    //             .context("setup persistent user namespace")?);
    //     }

    //     if shared_netns {
    //         if !shared_userns {
    //             Err(anyhow!("shared netns cannot be enabled without shared userns"))?
    //         }
    //         self.shared_netns = Some(Namespace::new(&self.logger)
    //             .get_net()
    //             .setup(self.shared_userns.as_ref())
    //             .await
