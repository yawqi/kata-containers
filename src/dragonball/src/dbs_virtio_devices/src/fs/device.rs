...
impl<AS: GuestAddressSpace> VirtioFs<AS> {
    ...
    pub fn new(
        ...
        let uid_mappings = uid_mappings.into_iter().map(|(internal_id, external_id, size)| VfsIdMapping::new(internal_id, external_id, size)).collect();
        let gid_mappings = gid_mappings.into_iter().map(|(internal_id, external_id, size)| VfsIdMapping::new(internal_id, external_id, size)).collect();
